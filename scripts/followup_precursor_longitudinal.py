...
def main() -> None:
    ...
    writer = csv.DictWriter(
        sys.stdout,
        fieldnames=[
            'variant', 'alpha', 'mu', 'a', 'gamma', 'm0', 'steps_short', 'steps_long',
            'regime_short', 'regime_long', 'final_M_short', 'final_M_long',
            'final_Xi_short', 'final_Xi_long', 'under_resolved_short', 'under_resolved_long',
        ],
    )
    writer.writeheader()
    ...
    if traj_path:
        ...
        traj_writer = csv.DictWriter(
            traj_file,
            fieldnames=['cell_id', 'variant', 'alpha', 'mu', 'a', 'gamma', 'm0', 't', 'M_t', 'Xi_t'],
        )
    ...
    for idx, r in enumerate(precursor_rows):
        ...
        params = ModelParams(alpha=alpha, a=a, mu=mu, beta=0.05, eta=0.02, tap_variant=args.variant)
