...
def filter_variant(rows: list[dict], variant: str) -> list[dict]:
    if not rows or "variant" not in rows[0]:
        return rows
    return [r for r in rows if r.get("variant", "baseline") == variant]
...
def main():
    ...
    sigma_rows = filter_variant(load_csv(args.sigma_feedback), args.variant)
    mode_a_rows = filter_variant(load_csv(args.mode_a), args.variant)
    mode_b_rows = filter_variant(load_csv(args.mode_b), args.variant)
